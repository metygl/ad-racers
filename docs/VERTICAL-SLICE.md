...
| ART-06 | Glasshouse has the best sky and the weakest exposure | **FIXED** | Exposure is per course and applies on the low tier too. Glasshouse: exposure 1.72, key 1.6, hemisphere fill 3.2, road lifted to `0x646d78`. A focused route light models the skiff, and emissive route markers keep the road readable through the night-course value range. |
...
| ART-10 | Title, setup, garage, results lack spectacle | **FIXED** | Setup and results now stage the selected crew's production race model, lit and turning behind the panel (`HeroStage.ts`). Course cards still show the actual centreline and shortcut, and crew cards retain the data-derived profile silhouette (`ui/garage.ts`). The finish remains an authored moment: the camera takes a low three-quarter orbit, the rider pose scales with the result, and the results screen waits out the beat before presenting the same machine under result lighting. Reduced motion keeps a shorter steady beat and a stationary three-quarter garage view. |
...
| # | Finding | Status | Evidence on this commit |
|---|---|---|---|
| F1 | Near-plane geometry occludes the chase view | **FIXED** | A raycast along the camera boom against scenery and obstacles pulls the camera in, eased in fast and released slowly. Scenery clears the whole run-off and dissolves per instance around the camera, covering both approach occlusion and the case where the camera is already inside a canopy. |
...
| F4 | Drift is numerically active but visually incoherent | **FIXED** | Chassis and companion pose against lateral load, the tier language is unmistakable, and the camera keeps the road framed through a slide. The explicit bound the finding asked for is `PHYSICS.bodyAngleMax`: past the configured bound of *settled* slip the attitude jets pull the nose back towards the direction of travel in proportion to the excess, so a flick, a landing or a hit still cross the body up freely and only a *held* angle is caught. Its measured value and three regression checks are documented in the note below. |
...
### Post-preview correction pass

The accepted preview debt and the live review's blocking findings are closed on
the current commit. User-visible control, classification, difficulty and garage
behavior is owned by `README.md`; the durable implementation contracts remain
at their code boundaries:

| Area | Authoritative contract and regression proof |
| --- | --- |
| Input edges, resume suppression and browser Back | `InputManager.ts`, `App.ts`, `tests/e2e/gamepad.spec.ts`, `tests/e2e/race.spec.ts` |
| Classification of projected finishers | `state.ts`, `race.ts`, `tests/unit/preview-debt.test.ts` |
| Banking, crest state and GPU ownership | `vehicle.ts`, `Terrain.ts`, `materials/ownership.ts`, `tests/unit/preview-debt.test.ts` |
| Recovery route context and Glasshouse readability | `simulation.ts`, `Renderer.ts`, `glasshouseVigil.ts`, `tests/unit/preview-debt.test.ts` |
| First-run focus, garage and finish presentation | `dom.ts`, `SetupScreen.ts`, `HeroStage.ts`, `tests/e2e/presentation.spec.ts`, `tests/e2e/race.spec.ts` |
| Crew tactics and Ace defensive behavior | `racers.ts`, `driver.ts`, `tests/unit/preview-debt.test.ts` |
| Browser-test pacing | `tests/e2e/support.ts`, `tests/e2e/gamepad.spec.ts` |
